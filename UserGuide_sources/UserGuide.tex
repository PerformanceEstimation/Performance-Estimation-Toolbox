...
\begin{document}
	...
	\begin{itemize}
		\item[04/2021] {\bf{}[New]} New type of constraints: the linear matrix inequalities (LMIs) for imposing a matrix to be positive semi-definite, using the new command \verb?AddLMIConstraint?.
		\item[04/2021] {\bf{}[New]} The routine \verb?solve? output the dual values of these new constraints in the form of a cell array \verb?dualvalues_LMIs?.
		\item[04/2021] {\bf{}[New]} Example section extended with the Decentralized Gradient Descent (DGD), for two different cases: when the communication matrix is a priori given and when it is not, in which case we consider entire classes of possible matrices.
		...
		\begin{table}[ht!]{
			...
			\begin{table}[ht!]{
				...
				\begin{table}[ht!]
					...
					P.AddLMIConstraint(M);% Add an LMI constraint for M (Matrix M has non-negative eigenvalues)
					\end{lstlisting}
