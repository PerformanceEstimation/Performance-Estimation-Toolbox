...
xm1     = P.StartingPoint();        % x_{-1} is some starting point
x0      = P.StartingPoint();        % x_{0} is some starting point
[xs,fs] = F.OptimalPoint();         % xs is an optimal point, and fs=F(xs)

% (3) Algorithm' parameters (see [1])
lam   = .1;     % if lam = 1, it is gradient descent, if lam = 0 it is the
...
% (4) Perform two iterations according to the notations in [1]
y0      = x0+gamma*(x0-xm1);
[g0,F0] = F.oracle(y0);
