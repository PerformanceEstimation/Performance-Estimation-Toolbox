...
fctParam.mu = 0.1;
returnOpt = 1;
[Fi,Fav,xsi,Fsi] = P.DeclareMultiFunctions(fctClass,fctParam,N,returnOpt);
...
P.AddMultiConstraints(@(Fi, Fsi) Fi-Fsi <= 1, F_saved(:,1), Fsi(:));
...
%P.PerformanceMetric(Ftot.value(xavLast)-Fs); % OR 
P.PerformanceMetric((xavLast - xs)^2);
